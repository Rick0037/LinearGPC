...
%% Registers
parameters.ProblemParameters.InputNumber = 44; %number of OutputNumber
parameters.ControlLaw.VarRegNumber = parameters.ProblemParameters.OutputNumber+parameters.ProblemParameters.InputNumber;
parameters.ControlLaw.VarRegNumber = 50; % variable registers and constante registers (operands)
...
for p=(parameters.ProblemParameters.OutputNumber+1):(parameters.ProblemParameters.OutputNumber+parameters.ProblemParameters.InputNumber)
    r{p} = ['s(',num2str(p-parameters.ProblemParameters.OutputNumber),')'];
end
...
for p=1:9
	s_eval{p} = ['s(',num2str(p),',T)'];
end
for p=1:9
	s_eval{p+9} = ['s(',num2str(p),',T-2)'];
end
for p=1:9
	s_eval{p+18} = ['s(',num2str(p),',T-6)'];
end
for p=1:9
	s_eval{p+27} = ['s(',num2str(p),',T-8)'];
end

	s_eval{37} = 'cos(2*pi*PHI^(-4)*T/8)';
        s_eval{38} = 'cos(2*pi*PHI^(-3)*T/8)';
        s_eval{39} = 'cos(2*pi*PHI^(-2)*T/8)';
        s_eval{40} = 'cos(2*pi*PHI^(-1)*T/8)';
        s_eval{41} = 'cos(2*pi*PHI^(1)*T/8)';
        s_eval{42} = 'cos(2*pi*PHI^(2)*T/8)';
        s_eval{43} = 'cos(2*pi*PHI^(3)*T/8)';
        s_eval{44} = 'cos(2*pi*PHI^(4)*T/8)';
