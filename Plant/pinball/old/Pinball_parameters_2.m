...
%% Registers
parameters.ProblemParameters.InputNumber = 36; %number of OutputNumber
parameters.ControlLaw.VarRegNumber = 39;
parameters.ControlLaw.VarRegNumber = 42; % variable registers and constante registers (operands)
...
for p=1:9
	s_eval{p+18} = ['s(',num2str(p),',T-4)'];
end
for p=1:9
	s_eval{p+27} = ['s(',num2str(p),',T-6)'];
end
