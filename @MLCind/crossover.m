...
%% First offspring - CINDIV1
  ...
  % pretesting
  NAN = logical(sum(isnan(cindiv1.control_points)));
  INF = logical(sum(isinf(cindiv1.control_points)));
  if NAN||INF
      cindiv1.cost = num2cell(MLC_parameters.BadValue*ones(1,numel(gamma)+2));
  end
...
%% Second offspring - CINDIV2
  ...
  % pretesting
  NAN = logical(sum(isnan(cindiv2.control_points)));
  INF = logical(sum(isinf(cindiv2.control_points)));
  if NAN||INF
      cindiv2.cost = num2cell(MLC_parameters.BadValue*ones(1,numel(gamma)+2));
  end
