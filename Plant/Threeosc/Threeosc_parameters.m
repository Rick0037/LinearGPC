function parameters = Threeosc_parameters()
	% Threeosc_parameters sets the parameters for an Three oscillators problem.
	% See also MLC.
...
%% Problem parameters
% Problem
    parameters.Name = 'Threeosc'; % mlc.save_matlab('GenN'); ,mlc.load_matlab('Toy','GenN_moins_un')
    parameters.EvaluationFunction = 'Threeosc'; % external for fortran or experimental
        % Problem variables
        ...
        % Intputs - Number of sensors and time dependent functions
            % si(t)
            ProblemParameters.NumberSensors = 6;
            ProblemParameters.Sensors = {'a1','a2','a3','a4','a5','a6'}; % name in the problem
            % hi(t)
        ...
        TDF = cell(1,ProblemParameters.NumberTimeDependentFunctions); %* Time define fuction
        for p=1:ProblemParameters.NumberSensors,Sensors{p} = ['s(',num2str(p),')'];end %* 数组内部添加字符串
        for p=1:ProblemParameters.NumberTimeDependentFunctions,TDF{p} = ['h(',num2str(p),')'];end %*
        ControlSyntax = horzcat(Sensors,TDF); %* 串联两个表 将TDF接到在Sensors的右边
        ...
        ProblemParameters.NPeriods = 50; % Number of periods
        ...
        ProblemParameters.InitialCondition = [0.1 0 0.1 0 0.1 0]; % on the limit cycle of the unstable oscillator.
...
%% Control law parameters
        ...
        ControlLaw.InstructionSize.InitMax=30;
        ControlLaw.InstructionSize.InitMin=5;
        ControlLaw.InstructionSize.Max=30;
        % Operators
        ControlLaw.OperatorIndices = [1:5,7:9];
            ...
            %                    - 12 power         (pow)
            %
        ControlLaw.Precision = 6; % Precision of the evaluation of the control law % to change also in my_div and my_log精度
        % Registers
            ...
            ControlLaw.CstRegNumber = 6;
            ControlLaw.CstRange = [repmat([-1,1],ControlLaw.CstRegNumber,1)]; % Range of values of the random constants 函数 repmat 用于复制和
            ...
            % Register initialization
                ...
                dC = maxC-minC; %*
                for p=NVR+1:RN %*
                    r{p} = num2str(dC(p-NVR)*rand+minC(p-NVR)); %* 给予后面四位的常熟变量随机的数值，将其数值变为字符串
                end %*
