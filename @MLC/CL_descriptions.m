...
for gen = GENS
    for ind=1:PopSize
        ...
        for p=OperatorIndices
            for c=1:OutputNumber
                SM = submat{c};
                OperatorsTab(cmpt,ind,c,p) = sum(SM(:,3)==p);
            end
        end
        % Constant
        for p=1:CstRegNumber
            for c=1:OutputNumber
                SM = submat{c};
                ConstantsTab(cmpt,ind,p) = sum(sum(SM(:,1:2)==VarRegNumber+p));
            end
        end
    end
    cmpt = cmpt+1;
...
cmpt = 1;
for geni = GENS
    SSi = sum(SensorsTab(cmpt,:,:,:),3);
    SSi = reshape(SSi,[],InputNumber);
...
cmpt = 1;
for geni = GENS
    OTi = sum(OperatorsTab(cmpt,:,:,:),3);
    OTi = reshape(OTi,[],maxOI);
